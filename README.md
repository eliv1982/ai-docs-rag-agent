...
+ URL indexing into Pinecone (fetch → embeddings → upsert → verify → cleanup) implemented.
Retrieval, RAG generation, agent/tools и Telegram-интерфейс ещё не реализованы.**
...
Pinecone (`DocumentIndexingService.index_url`, см. ниже). Retrieval (семантический поиск),
генерация ответов через LLM, LangChain agent, tools и Telegram-интерфейс пока не
реализованы.
...
по-прежнему содержат только module docstring. **Retrieval (семантический поиск) и RAG
generation ещё не реализованы** — этот этап только готовит данные в Pinecone.
...
- `pytest` (`tests/test_config.py`, `tests/test_models.py`, `tests/test_pinecone_store.py`,
  ...
  `tests/test_index_url_script.py`) — быстрые unit-тесты на fake-объектах, без сети, без
  ...
  dependency-injection-friendly fakes (`DocumentIndexingService` и `PineconeStore` принимают
  fake-сервисы/клиенты и injectable clock/sleep).
